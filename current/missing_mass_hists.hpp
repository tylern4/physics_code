...
TH1D *Missing_Mass_cut = new TH1D("Missing_Mass_cut", "Missing Mass cut", bins_MM, MM_min, MM_max);

void Fill_Missing_Mass(double miss_mass){
	Missing_Mass->Fill(miss_mass);
}

void Fill_Missing_Mass_cut(double miss_mass){
	Missing_Mass_cut->Fill(miss_mass);
}
...
void Write_Missing_Mass(){
	...
	Missing_Mass_cut->SetXTitle("Mass (GeV)");
	Missing_Mass_cut->Write();
}
