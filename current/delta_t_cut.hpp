...
void delta_t_cut(){
	...
	double electron_vertex = delta_t::vertex_time(sc_t[sc[0]-1], sc_r[sc[0]-1], 1.0);
	...
	for(int event_number = 0; event_number < gpart; event_number++){
		...
		delta_t_P = delta_t::delta_t_calc(electron_vertex, MASS_P, p[event_number], sc_t[sc[event_number]-1], sc_r[sc[event_number]-1]);
		delta_t_PIP = delta_t::delta_t_calc(electron_vertex, MASS_PIP, p[event_number], sc_t[sc[event_number]-1], sc_r[sc[event_number]-1]);
		delta_t_ELECTRON = delta_t::delta_t_calc(electron_vertex, MASS_E, p[event_number], sc_t[sc[event_number]-1], sc_r[sc[event_number]-1]);
