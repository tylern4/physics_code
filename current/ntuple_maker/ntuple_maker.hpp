...
#ifndef NUTPLE_MAKER_H_GUARD
#define NUTPLE_MAKER_H_GUARD
#include "main.h"
...
void make_electron_csv(char *fin, char *csv_name_output) {
  ...
  for (int current_event = 0; current_event < num_of_events; current_event++) {
    ...
    electron_cuts &= (data->dc_stat[dc[0] - 1] > 0);

    csv_output << data->(data->id[0] == ELECTRON) << ",";
    csv_output << data->gpart << ",";
    csv_output << data->ec[0] << ",";
    csv_output << data->stat[0] << ",";
    csv_output << data->q[0] << ",";
    csv_output << data->sc[0] << ",";
    csv_output << data->dc[0] << ",";
    csv_output << data->dc_stat[dc[0] - 1] << ",";
    ...
    if (electron_cuts && data->cc[0] > 0) {
      cc_sector = cc_sect[data->cc[0] - 1];
      cc_segment = (data->cc_segm[0] % 1000) / 10;
      cc_pmt = data->cc_segm[0] / 1000 - 1;
      cc_nphe = data->nphe[data->cc[0] - 1];
      theta_cc = TMath::ACos(TMath::Abs(p[0] * cz[0]) / TMath::Abs(p[0]));
      theta_cc = theta_cc / D2R;
    }
    csv_output << data->cc[0] << ",";
