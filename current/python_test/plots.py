...
class plotting(object):
    ...
    def hist2D(self, x_val, y_val, **kwargs):
        ...
        fig = plt.figure(num=None, figsize=self.fig_size,
                         dpi=200, facecolor='w', edgecolor='k')
        if 'range' in kwargs:
            plt.hist2d(x_val, y_val, bins=self.num_bins,
                       range=kwargs['range'], cmap=self.my_cmap)
        else:
    ...
    def hist1D(self, x_val, **kwargs):
        ...
        if self.pickle:
            pl.dump(fig, open(self.output + self.plot_name +
                              '.pkl', 'wb'), protocol=2)
        if self.save:
            print("Output to " + self.output + self.plot_name)
            plt.savefig(self.output + self.plot_name)
