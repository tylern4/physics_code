...
//
void WvsQ2_test(char *fin, char *RootFile_output){
	...
	for (int current_event = 0; current_event <= num_of_events; current_event++) {
		...
		TVector3 e_mu_prime_3; //= new TVector3;
		TLorentzVector e_mu_prime; //= new TLorentzVector;
		...
		if (id[0] == ELECTRON && gpart > 1 && stat[0] > 0 && (int)q[0] == -1 && cc[0] > 0 && sc[0] > 0 && dc[0] > 0 && ec[0] > 0 && dc_stat[dc[0]-1] > 0 ){
			//Setup scattered electron 4 vector
			e_mu_prime_3->SetXYZ(p[0]*cx[0],p[0]*cy[0],p[0]*cz[0]);	
			e_mu_prime->SetVectM(*e_mu_prime_3, MASS_E);
			//e_mu_prime.SetVectM(e_mu_prime_3, m[0]);

			cout << Q2_calc(e_mu, e_mu_prime) << endl;
