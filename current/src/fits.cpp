...
void Fits::Set_min(double val) { this->min_value = val; };
void Fits::Set_max(double val) { this->max_value = val; };

double Fits::Get_min_edge() { return this->min_edge_x; }
double Fits::Get_max_edge() { return this->max_edge_x; }
double Fits::Get_sigma() { return this->sigma; }
double Fits::Get_mean() { return this->mean; }
double Fits::Get_FWHM() { return this->FWHM; }

void Fits::FitGaus(TH1D *hist) {
  if (hist->GetEntries() > 1000) {
    // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
    TF1 *fitFunc = new TF1("fitFunc", gaus.c_str(), this->min_value, this->max_value);
    ...
    hist->Fit("fitFunc", "q", "", this->min_value, this->max_value);
    ...
    for (int i = 0; i < 10; i++) hist->Fit("fitFunc", "q", "", this->min_value, this->max_value);
...
void Fits::FitLandau(TH1D *hist) {
  if (hist->GetEntries() > 1000) {
    // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
    TF1 *fitFunc = new TF1("fitFunc", "landau", this->min_value, this->max_value);
...
void Fits::FitPoly_1D(TH1D *hist) {
  // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
  TF1 *fitFunc = new TF1("fitFunc", ploy1d.c_str(), this->min_value, this->max_value);
...
void Fits::FitPoly_2D(TH1D *hist) {
  // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
  TF1 *fitFunc = new TF1("fitFunc", ploy2d.c_str(), this->min_value, this->max_value);
...
void Fits::FitPoly_3D(TH1D *hist) {
  // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
  TF1 *fitFunc = new TF1("fitFunc", ploy3d.c_str(), this->min_value, this->max_value);
...
void Fits::FitPoly_4D(TH1D *hist) {
  // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
  TF1 *fitFunc = new TF1("fitFunc", ploy4d.c_str(), this->min_value, this->max_value);
...
void Fits::FitPoly_fid(TH2D *hist) {
  // ROOT::Math::MinimizerOptions::SetDefaultMinimizer("Minuit2");
  TF1 *fitFunc = new TF1("fitFunc", "pol8", this->min_value, this->max_value);
  fitFunc->SetLineColor(42);
...
void Fits::FitFiducial_lo(TH2D *hist2d) {
  ...
  hist2d->Fit("fitFunc_lo", "q", "", this->min_value, this->max_value);
...
void Fits::FitFiducial_hi(TH2D *hist2d) {
  ...
  hist2d->Fit("fitFunc_hi", "q", "", this->max_value, this->max_value);
...
void Fits::FitFiducial(TH2D *hist2d) {
  ...
  hist2d->Fit("fitFunc", "q", "", this->max_value, this->max_value);
...
void Fits::FitGenNormal(TH1D *hist) {
  ...
  if (hist->GetEntries() > 1000) {
    ...
    TF1 *fitFunc = new TF1("genNormal", physics::genNormal, this->min_value, this->max_value, 4);

    fitFunc->SetParLimits(1, 5.0, 200.0);

    fitFunc->SetParameter(0, 15.0);
    fitFunc->SetParameter(1, 10.0);
    fitFunc->SetParameter(2, (this->min_value + this->max_value) / 2.0);
    fitFunc->SetParameter(3, 3000.0);
    fitFunc->SetParNames("alpha", "beta", "mu", "weight");

    hist->Fit("genNormal", "Q+", "", this->min_value, this->max_value);

    for (double m = this->min_value; m < this->max_value; m = m + 0.005) {
