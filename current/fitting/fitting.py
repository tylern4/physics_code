...
def __fit1(binned_func, df, bound, bins, sector):
    ...
    model = AddPdf(gaussian, rtv_breitwigner)
...
def __fit(binned_func, df, bound, bins, sector):
    ...
    binned_function = binned_func(model, fit_data, bins=bins, bound=bound)
    minuit = Minuit(binned_function, m=0.94, gamma=0.03,
                    error_m=0.1, error_gamma=0.001)
    minuit.migrad()
    try:
        print(minuit.minos())
    except Exception as e:
...
if __name__ == '__main__':
    ...
    df = root_pandas.read_root(
        "/Users/tylern/Data/ntuple/ntuple.root", "ntuple")
