// 
// Fit1 - background subtraction routine where the peak is fit with a well-known function
...
void PolSubtraction(char *rootFile, char *hname, char* yieldFile)
{
...
*/
	...
	hBgFit->GetXaxis()->SetRangeUser(xLo,xHi);
	hBgFit->Draw("same");
	//draw peak
	Peak->GetXaxis()->SetRangeUser(xLo,xHi);
	Peak->SetFillColor(4);
	Peak->Draw("same");
