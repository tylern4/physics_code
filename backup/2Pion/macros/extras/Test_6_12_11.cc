...
if(temp.tgt_doca>14)cout<<"8:"<<temp.tgt_doca<<endl;
...
void PolSubtractionAngle2(char *rootFile, char* yieldFile){
	...
	for(i=0;i<=count;i++){
	   ...
	   switch (ii){
	      case 1: sprintf(hname, "AngleCutAroundTgt_range_%d_tgt_%s",i, tar);
	              sprintf(AAA,"Angle"); 
	              break;
	      case 2: sprintf(hname, "MassCutAroundTgt_%s_%d",tar,i);
	              sprintf(AAA,"Around"); 
	              break;
	      case 3: sprintf(hname, "MassCutAfterTgt_%s_%d",tar,i);
	              sprintf(AAA,"After"); 
	              break;}
...
TF1 *pol = new TF1("pol",totFit,xLo,xHi,PolNum);
	   ...
	   char OutCan[100];
	   sprintf(OutCan,"pol/%s_%d.gif",yieldFile,i);
	   can1->Print(OutCan);
	   sprintf(OutCan,"pol/%s_%d.eps",yieldFile,i);
	   can1->Print(OutCan);

	   // open text file for the yields
	   char OutFile[100];

	   sprintf(OutFile,"pol/%s_%d.yld",yieldFile,i);
	   ofstream fout(OutFile);
 
	   yields[j][i] = yield;
           if(i == 0) yield_2H = yield;
	   ratio[i] = yield/yield_2H;
	   ...
	   //cout<<"\t"<<yield<<"\t"<<sqrt(yield)<<"\t"<<ratio[i]<<endl;
        ...
        sprintf(title,"ratio around %s %s",tar,AAA);
	if(ii==1){
	RatioHist[j] = new TH1F(hname,title, 360, 0, 180 );
	for(i=0;i<=count;i++){RatioHist[j]->SetBinContent(i*10, ratio[i]);}}

	if(ii!=1){
	RatioHist[j] = new TH1F(hname,title, 72, 0, 36 );
	for(i=0;i<=count;i++){ RatioHist[j]->SetBinContent((i*10)+1, ratio[i]);}}
      ...
      for(j=0;j<nType;j++){                                     // loop over vz type, [0,in target], [1,after target]
	   ...
	   Int_t tar = 0;
	      ...
	      for(k=0; k<=36; k++){
		    ...
		    AngleCutAroundTarget[0][k]->Fill(pairM);
	            AngleCutAroundTarget[tar][k]->Fill(pairM);}}
	  }
...
int main(int argc, char **argv){
	...
	BgSub_Sideband(outfilename,"test");
        PolSubtractionAngle2(outfilename, "test2");
  ...
  for(j=0;j<=36;j++){
    for(jj=0;jj<=4;jj++){     
       if(jj==0) target = "all";
        ...
        sprintf(hname,"AngleCutAroundTgt_range_%d_tgt_%s", j,target);
        sprintf(title,"Angle Cut Around %s range %d",target,j);
