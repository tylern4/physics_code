  ...
  hVz = new TH2F(hname,title,nVz,zlo,zhi,2,-0.5,1.5);
   ...
   double range, range2;
      ...
      for(k=1;k<=6;k++){
        ...
        for (i=1;i<=4;i++){
           sprintf(hname,"MassCutAroundTgt%i%i",i,k);
           sprintf(title,"Mass Cut Around Target %i Momentums between %s",i,between);
           MassCutAroundTgt[i][k] = new TH1F(hname,title,400, 0, 2); 
        
           sprintf(hname,"MassCutAfterTgt%i%i",i,k);
           sprintf(title,"Mass Cut After Target %i Momentums between %s",i,between);
