...
void BookHist(){
  ...
  hVz = new TH2F(hname,title,nVz,zlo,zhi,2,-0.5,1.5);
   ...
   for(k=1;k<=6;k++){
     ...
     MomentumCutAngle[k] = new TH1F(hname,title,60, range, range2);
...
void PolSubtractionAngle(char *rootFile, char* yieldFile){
	...
	for(i=0;i<=count;i++){		//for loop for difffernt angle ranges
	   ...
	   YieldHist[tar_c] = new TH1F(hname,title, 380, 0, 190 );
	   ...
	   ChiHist[tar_c] = new TH1F(hname,title, 380, 0, 190 );
	   ...
	   RatioHist[tar_c] = new TH1F(hname,title, 380, 0, 190 ); 
	...
	Ratio[meth_c] = new TH1F(hname,title, 210, 0, 210 );
	...
	Stuff[meth_c] = new TH1F(hname,title, 210, 0, 210 );
