...
/***/
  ...
  void PolSubtractionWeighted(char *rootFile, char* yieldFile);
...
/***/
     Int_t order[nFoils];
	  Double_t rtgt_non[nFoils];
     Double_t zdiff_non[nFoils];
     Double_t decayL_non[nFoils];
     Double_t rdiff_non[nFoils];
     Double_t ctau_non[nFoils];
     Double_t norm_weigh[nFoils];};
/***/	
  // pos. - does not cross beamline, neg. - crosses beamline
       ...
       if(vz>zFoil_mean[i]){
         ...
         if(fabs(temp.rtgt[i])<0.6){ 
         	...
         	if(fabs(temp.rtgt[i])<fabs(temp.rtgt[i-1])){
			   ...
			   temp.order[temp.nTgtTrav] = i;
            temp.rtgt_non[temp.nTgtTrav] = temp.rtgt[i];
            temp.decayL_non[temp.nTgtTrav] = temp.decayL[i];
            temp.zdiff_non[temp.nTgtTrav] = temp.zdiff[i];
            temp.rdiff_non[temp.nTgtTrav] = temp.rdiff[i];
            temp.ctau_non[temp.nTgtTrav] = temp.ctau[i];
	...
	i = 0; // inititalize the event counter for each file
	 ...
	 if (cuts && tgt[j]){
	   ...
	   TgtInfo = GetParticleTargetInfo(pairZ, sqrt(pairX*pairX+pairY*pairY), pairV4);
		...
		Double_t weight[TgtInfo.nTgtTrav];
...
/***/
					...
					for(t=0;t<TgtInfo.nTgtTrav;t++){
					weight[t] = (1/exp(TgtInfo.decayL_non[t]/2.6));  //divided by the decay length in cm
					sum += weight[t];
				}

				for(t=0;t<TgtInfo.nTgtTrav;t++){
					if(TgtInfo.order[t]!=0) MassTgtWeighted[TgtInfo.order[t]]->Fill(pairV4.M(),weight[t]/sum);
				}
/***/
...
int main(int argc, char **argv){
   ...
   PolSubtractionAngle(outfilename, "PolSubtractionAngle");
	...
	PolSubtractionWeighted(outfilename,"PolSubtractionWeighted");
...
/***/
     for (i=1;i<=8;i++){
       ...
       sprintf(hname,"MassWeighted_%s",target);
       sprintf(title,"Weighted Mass Hist for %s",target);
       MassTgtWeighted[i] = new TH1F(hname,title,10000, 0, 2);
...
fout<<"yields(for "<<tar<<")\t\tyields[0]"<<endl;
fout<<yields[tar_c][meth_c][0]<<"\t\t"<<yields[0][meth_c][0]<<endl;
...
cout<<"fd\t"<<fd<<endl;
	...
	sprintf(name, "MassCut%sTgt_%s",AAA,tar);
	   ...
	   // histogram of background
	   MomhBgFit = (TH1F*)Momhist->Clone("hfBgFit"); // clone original hist. into background temp. hist.
...
void PolSubtractionWeighted(char *rootFile, char* yieldFile){
	...
	TH1F *Wgthist; // original histogram
	TH1F *WgthBgFit; // histogram of background
	TH1F *WgtPeak;  //histogram of peak
	TH1F *WgtYieldHist[8];
	TH1F *WgtChiHist[8];
	TH1F *WgtRatioHist[8];
	TH1F *WgtRatio[8];
	...
	TFile *fd = new TFile(rootFile,"UPDATE"); 	// open up the ROOT file
	   ...
	   for(tar_c=0;tar_c<=3;tar_c++){		//for loop for different targets
	      ...
	      switch (tar_c){ 
	         case 0: tar = "2H";
						  Wgt2H = (TH1F*)fd->Get("MassWeighted_2H");
						  Wgthist = (TH1F*)Wgt2H;
	                 break;
	         case 1: tar = "C";
						  WgtCarbon[0] = (TH1F*)fd->Get("MassWeighted_C1");
						  WgtCarbon[1] = (TH1F*)fd->Get("MassWeighted_C2");
						  WgtCarbon[2] = (TH1F*)fd->Get("MassWeighted_C3");
						  WgtCarbon[3] = (TH1F*)fd->Get("MassWeighted_C4");
						  WgtCarbon[0]->Add(WgtCarbon[1],1);
						  WgtCarbon[0]->Add(WgtCarbon[2],1);
						  WgtCarbon[0]->Add(WgtCarbon[3],1);
	  					  Wgthist = (TH1F*)WgtCarbon[0];
	                 break;
	         case 2: tar = "FeTi";
						  WgtFeTi[0] = (TH1F*)fd->Get("MassWeighted_Fe");
						  WgtFeTi[1] = (TH1F*)fd->Get("MassWeighted_Ti");
						  WgtFeTi[0]->Add(WgtFeTi[1],1);
					     Wgthist = (TH1F*)WgtFeTi[0];
	                 break;
	         case 3: tar = "Pb";
						  WgtPb = (TH1F*)fd->Get("MassWeighted_Pb");
						  Wgthist = (TH1F*)WgtPb;
	                 break;

	      }

		sprintf(name, "MassWeighted_%s",tar);

				
	   // histogram of background
	   WgthBgFit = (TH1F*)Wgthist->Clone("hBgFit"); // clone original hist. into background temp. hist.
	   WgthBgFit->SetName("hBgFit");
	   WgthBgFit->SetTitle("Background");

	   WgtPeak = (TH1F*)Wgthist->Clone("Peak");
	   WgtPeak->SetName("Peak");
	   WgtPeak->SetTitle("Peak");

	   SumLo = 0.486;
	   SumHi = 0.507;			
	   Xbins = WgthBgFit->GetNbinsX();


	   for(k=1; k<Xbins; k++){
	      if (WgthBgFit->GetBinCenter(k)<=SumHi && WgthBgFit->GetBinCenter(k)>=SumLo){ 
	         WgthBgFit->SetBinContent(k,0.0);//Set all bins inside peak range to be 0
	      }
	   }

	   TF1 *pol = new TF1("pol",polFit,xLo,xHi,PolNum);

	   WgthBgFit->Fit("pol","R+");         // fit the background
	   pol->GetParameters(&par[ibg]);
	   pol->SetParameters(&par[ibg]); 	// set the pfinal parameters for the background function
	   NDF = pol->GetNDF();
	   Chi[i] = pol->GetChisquare();
	   ...
	   WgtPeak->Add(pol,-1.0); 		// subtract background function

	   for(k=1; k<WgtPeak->GetNbinsX(); k++){
		x = WgtPeak->GetBinCenter(k); 		// read bin center value on the x-axis
		if(x>=SumLo && x<=SumHi){ 		// check that x is in the peak summation region
			pval = WgtPeak->GetBinContent(k);	// get the number of counts in the bin
			if(pval<0.0) pval = 0.0; 	// if neg. counts, set to zero
		}
		else{
			pval =0.0;
		}
		WgtPeak->SetBinContent(k,pval); // refill the histogram
	      }

	      yields[tar_c] = WgtPeak->Integral(WgtPeak->FindBin(SumLo),WgtPeak->FindBin(SumHi)); //sum total counts in peak
	      ...
	      // draw the original histogram
	      Wgthist->SetTitle(0);
	      Wgthist->GetXaxis()->SetTitle(xtitle);
	      Wgthist->GetXaxis()->CenterTitle();
	      Wgthist->GetYaxis()->SetTitle(ytitle);
	      Wgthist->GetYaxis()->CenterTitle();
	      Wgthist->GetYaxis()->SetTitleOffset(yoff);
	      Wgthist->GetXaxis()->SetRangeUser(xLo,xHi);  // set the x-axis range for the plot
	      Wgthist->SetLineWidth(1);
	      Wgthist->SetMinimum(0); // start the y-axis at zero.
	      Wgthist->Draw();
	      ...
	      //draw peak
	      WgtPeak->SetFillColor(4);
	      WgtPeak->Draw("same");
	
	
	      // create the image files
	      char OutCan[100];
	      sprintf(OutCan,"pol/%s_%s.gif",yieldFile,tar);
	      can1->Print(OutCan);
	      sprintf(OutCan,"pol/%s_%s.eps",yieldFile,tar);
	      can1->Print(OutCan);


	      // open text file for the yields
	      char OutFile[100];
	      sprintf(OutFile,"pol/%s_%s.yld",yieldFile,tar);
	      ofstream fout(OutFile);


	      if(yields[tar_c] >= 1 && yields[0] >= 1) ratio[i] = yields[tar_c]/yields[0];

	      if(yields[tar_c]<1 && yields[0]<1)ratio[i] = 0;


	   sprintf(hname2,"Yield_Wgt_%s_%s",name,tar);
      sprintf(title,"Yield weighted for %s target",tar);
	   WgtYieldHist[tar_c] = new TH1F(hname2,title, 30, 0, 15 );
	   WgtYieldHist[tar_c]->GetYaxis()->SetTitle("counts");
	   WgtYieldHist[tar_c]->GetYaxis()->CenterTitle();
	   WgtYieldHist[tar_c]->GetXaxis()->SetTitle("weighted range");
	   WgtYieldHist[tar_c]->GetXaxis()->CenterTitle();


	   sprintf(hname2,"Chi_Wgt_%s",name,tar_c);
      sprintf(title,"Chi-Square for target %s",tar);
	   WgtChiHist[tar_c] = new TH1F(hname2,title, 30, 0, 15 );
	   WgtChiHist[tar_c]->GetYaxis()->SetTitle("Chi-Square Value");
	   WgtChiHist[tar_c]->GetYaxis()->CenterTitle();
	   WgtChiHist[tar_c]->GetXaxis()->SetTitle("weighted range");
	   WgtChiHist[tar_c]->GetXaxis()->CenterTitle();
	   ...
	   for(i=0;i<=count;i++){
	      WgtChiHist[tar_c]->SetBinContent((i*4)+1, Chi[i]);
	      WgtYieldHist[tar_c]->SetBinContent((i*4)+1, yields[tar_c]);
	   }

		fout<<"yields(for "<<tar<<")\t\tyields[0]"<<endl;
		fout<<yields[tar_c]<<"\t\t"<<yields[0];


	   sprintf(hname2,"Ratio_Wgt_%s",name);
      sprintf(title,"Ratio for %s target weighted",tar);
	   WgtRatioHist[tar_c] = new TH1F(hname2,title, 30, 0, 15 ); 
	   WgtRatioHist[tar_c]->GetYaxis()->SetTitle("ratio");
	   WgtRatioHist[tar_c]->GetYaxis()->CenterTitle();
	   WgtRatioHist[tar_c]->GetXaxis()->SetTitle("weighted range");
	   WgtRatioHist[tar_c]->GetXaxis()->CenterTitle();


	   for(i=0;i<=count;i++){
	      WgtRatioHist[tar_c]->SetBinContent((i*4)+1, ratio[i]);}

	   WgtRatioHist[tar_c]->Scale((A[0]*d[0]*Aw[tar_c])/(A[tar_c]*d[tar_c]*Aw[0]));	
	   WgtRatioHist[tar_c]->Write();
           WgtYieldHist[tar_c]->Write();
	   WgtChiHist[tar_c]->Write();

	}//end for tar_c targets

	   gPad->SetLogy();
	   WgtYieldHist[0]->GetXaxis()->SetTitle("angle in degrees");
	   WgtYieldHist[0]->GetXaxis()->CenterTitle();
	   WgtYieldHist[0]->SetTitle("Ratio of Yeilds to 2H");   
  	   WgtYieldHist[3]->SetMarkerSize(1.25);
  	   WgtYieldHist[3]->SetMarkerStyle(22);
	   WgtYieldHist[3]->SetMarkerColor(5);
	   WgtYieldHist[3]->Draw("p");
  	   WgtYieldHist[2]->SetMarkerSize(1.25);
  	   WgtYieldHist[2]->SetMarkerStyle(22);
	   WgtYieldHist[2]->SetMarkerColor(4);  	   
	   WgtYieldHist[2]->Draw("p""same");
  	   WgtYieldHist[1]->SetMarkerSize(1.25);
  	   WgtYieldHist[1]->SetMarkerStyle(22);
	   WgtYieldHist[1]->SetMarkerColor(3);  	   
	   WgtYieldHist[1]->Draw("p""same");
  	   WgtYieldHist[0]->SetMarkerSize(1.25);
  	   WgtYieldHist[0]->SetMarkerStyle(22);
  	   WgtYieldHist[0]->SetMarkerColor(2);
	   WgtYieldHist[0]->Draw("p""same");
	   ...
	   gPad->SetLogy();
	   WgtRatioHist[0]->GetXaxis()->SetTitle("angle in degrees");
	   WgtRatioHist[0]->GetXaxis()->CenterTitle();
	   WgtRatioHist[0]->SetTitle("Ratio of Yeilds to 2H");
  	   WgtRatioHist[3]->SetMarkerSize(1.25);
  	   WgtRatioHist[3]->SetMarkerStyle(22);
	   WgtRatioHist[3]->SetMarkerColor(5);
	   WgtRatioHist[3]->Draw("p");
  	   WgtRatioHist[2]->SetMarkerSize(1.25);
  	   WgtRatioHist[2]->SetMarkerStyle(22);
	   WgtRatioHist[2]->SetMarkerColor(4);  	   
	   WgtRatioHist[2]->Draw("p""same");
  	   WgtRatioHist[1]->SetMarkerSize(1.25);
  	   WgtRatioHist[1]->SetMarkerStyle(22);
	   WgtRatioHist[1]->SetMarkerColor(3);  	   
	   WgtRatioHist[1]->Draw("p""same");
  	   WgtRatioHist[0]->SetMarkerSize(1.25);
  	   WgtRatioHist[0]->SetMarkerStyle(22);
  	   WgtRatioHist[0]->SetMarkerColor(2);
	   WgtRatioHist[0]->Draw("p""same");
