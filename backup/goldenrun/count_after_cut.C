...
void count_after_cut() {
  ...
  string s, str_runnum, str_filenum; // make variables s="string name from input
                                     // file list,temperary name for each time
                                     // through loop" str_runnum=??
                                     // str_filenum=??
  TFile *rootfile;
  TTree *tree;
  ...
  // while loop to read through input file. Last line says end on it to break
  // the loop
  while (infile >> s) {
    ...
    for (Long64_t i = 0; i < nentries; i++) { // My while loop
      ...
      if ((q_temp > 0.)) {
        ...
        if ((qcurr > qprev)) {
          deltaq = qcurr - qprev;
          totalQ += deltaq;
          cout << "qcurr=" << qcurr << "qprev=" << qprev << "deltaq=" << deltaq
               << endl;
      ...
      for (int j = 0; j < gpart; j++) {
        ...
        if (ef_4vec != ZERO_4vec && pf_4vec != ZERO_4vec &&
          ...
          // if( (im_cor<1.116-3*0.001761) || (im_cor>1.116+3*0.001761) )
          // continue;
    ...
    cout << str_runnum << "    " << str_filenum << "    " << n_event
         << "totalQ=" << totalQ << endl;
    if (n_event != 0 && totalQ != 0) {
      outfile << setw(20) << setiosflags(ios::left) << str_runnum << setw(20)
              << setiosflags(ios::left) << str_filenum << setw(20)
              << setiosflags(ios::left) << n_event << setw(20)
              << setiosflags(ios::left) << totalQ << setw(20)
              << setiosflags(ios::left) << n_event / totalQ << endl;
    }
  }
  outfile.close();
}
