...
void plot_golden() {
  ...
  TH2F *hist = new TH2F("Ratio Vs Filenum_hist",
                        "Ratio vs Filenum after cut;File num;Event/Charge", 50,
                        3650000, 3662000, 50, 10000, 20000);
  TH1F *hist1 =
      new TH1F("Ratio", "Ratio after cut;Event/Charge", 50, 10000, 20000);
  ...
  TCanvas *canv =
      new TCanvas("Ratio Vs Filenum", "Ratio Vs Filenum after cut", 1000, 600);
  gPad->SetLogz();
  hist->Draw("colz");
  canv->SaveAs("ratio vs filenum-----after cut.ps");

  TCanvas *canv_hist1 =
      new TCanvas("Ratio dustribution", "Ratio distribution", 1000, 600);
  gPad->SetLogz();
  hist1->Fit(gaus1, "R");
  hist1->Draw();
  ...
  TLine *line1_1 =
      new TLine(mean1 - n_sigma * sigma1, 0, mean1 - n_sigma * sigma1, 100);
  ...
  TCanvas *canv1 = new TCanvas("Ratio Vs Filenum --- graph",
                               "Ratio Vs Filenum after cut graph", 1000, 600);
  gPad->SetLogz();
  gr->Draw("ap");
  ...
  outfile1<<"a1ntp_"<<runnum<<"_pass1.a"<<filenum<<".rzn.root"<<endl;
